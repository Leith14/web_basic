...
let reg: RegExp = new RegExp("\\d+", "g");
console.log(num, date, reg);
let error: Error = new Error("错误信息");
...
let div4: NodeListOf<HTMLDivElement | HTMLElement> =
  document.querySelectorAll("div");

let local: Storage = localStorage;
let loc: Location = location;

let promise:Promise<string> = new Promise((r) => {
  r('leixw');
});
promise.then((res)=>{
    console.log(res);
...
let str: string[] = "leixwxkeifd090801011".split("");
